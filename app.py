...
# --- Deseasonalize Option ---
st.subheader("🔧 Ticket Data Processing")

apply_deseason = st.checkbox("Deseasonalize ticket data using STL?", value=True)

deseason_method = st.radio(
    "Select ticket metric to analyze:",
...
target_col = deseason_method  # default to raw

if apply_deseason:
    st.write("Using STL decomposition to remove weekly seasonality...")
    ...
    st.write("✅ Using raw ticket metric without deseasonalizing.")
    target_col = deseason_method  # use raw
...
ax1.set_title(f"Correlation: {vix_metric} vs {target_col}")
ax1.grid(alpha=0.3)
st.pyplot(fig1)
...
if plot_data.empty:
    ...
    ax2.legend()
    ax2.set_title("Normalized VIX vs Ticket Metric")
...
# Insight
processing_type = "deseasonalized" if apply_deseason else "raw"
if best_corr > 0.1:
    insight = (
        f"There is a {strength_desc} positive correlation ({best_corr:.3f}) between "
        f"**{vix_selected}** and {processing_type} **{deseason_method}** at lag {best_lag} day(s). "
        f"Higher volatility tends to precede increased ticket volume."
    )
elif best_corr < -0.1:
    insight = (
        f"There is a {strength_desc} negative correlation ({best_corr:.3f}) between "
        f"**{vix_selected}** and {processing_type} **{deseason_method}** — high VIX linked to lower activity."
    )
else:
    insight = (
        f"No meaningful correlation was found (best = {best_corr:.3f}). "
        f"Market volatility does not appear to drive {processing_type} ticket volume in this period."
    )

# Display summary
st.markdown(f"""
- **📅 Analysis Period**: {date_from} to {date_to}
- **📊 Ticket Metric**: `{deseason_method}`
- **🔧 Processing**: {'Deseasonalized using STL' if apply_deseason else 'Raw (no adjustment)'}
- **📈 VIX Metric Tested**: `{vix_selected}`
- **⏰ Best Lag**: {best_lag} day(s)
- **🔗 Correlation Strength**: **{best_corr:.3f}** → {strength_desc} relationship {emoji}
- **💡 Insight**: {insight}

> 💬 **Conclusion**: `{vix_selected}` at lag {best_lag} explains **{(best_corr**2)*100:.1f}%** of the variation in {processing_type} `{deseason_method}`.
""")
...
st.download_button(
    label="Download merged & processed data as CSV",
    data=csv,
...
### 🧹 Why Deseasonalize? (Optional)
- Tickets often drop on weekends.
- STL removes weekly patterns to expose true anomalies.
- **If unchecked**, raw values are used — useful for short-term or aggregated analysis.
...
- Toggle deseasonalization to compare results
- Update monthly
